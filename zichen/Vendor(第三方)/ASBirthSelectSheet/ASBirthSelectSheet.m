...
- (void)setDateModeWithMode:(UIDatePickerMode ) model andFormaString:(NSString *)string{
    [self.formatter setDateFormat:string];
    [_datePicker setDatePickerMode:model];
...
//    [[NSNotificationCenter defaultCenter]postNotificationName:@"timeForLeaving" object:nil];
    if (self.GetSelectDate) {
        _GetSelectDate([self.formatter stringFromDate:_datePicker.date],_datePicker.date);
        [self removeFromSuperview];
    }
}
